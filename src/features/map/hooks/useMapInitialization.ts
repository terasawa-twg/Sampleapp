...
import type {
  GeoloniaMap,
  GeoloniaMapEvent,
  HeartRailsGeoAPIResponse,
  GeoloniaMarker,
} from "../types/geolonia.types";
  ...
  const handleMapClick = useCallback((e: GeoloniaMapEvent) => {
    ...
    const processClick = async () => {
      try {
        ...
        if (loc) {
          console.log(
            `住所: ${loc.prefecture}${loc.city}${loc.town} (${loc.postal})`,
          );
        } else {
    ...
    const initializeMap = () => {
      ...
      console.log(
        ...
        Object.keys(window).filter((key) =>
          key.toLowerCase().includes("geolonia"),
        ),
      );
      ...
      if (window.geoloniaEmbed) {
        console.log(
          "window.geoloniaEmbed の構造:",
          Object.keys(window.geoloniaEmbed),
        );
