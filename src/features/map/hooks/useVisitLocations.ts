...
function transformDbLocationToVisitLocation(
...
export function useVisitLocations() {
  ...
  const locations: VisitLocation[] = dbLocations
    ? dbLocations.map((loc) => {
        ...
        const dbLoc = {
          ...loc,
          latitude:
            typeof loc.latitude === "object" && "toNumber" in loc.latitude
              ? Number(loc.latitude.toNumber())
              : Number(loc.latitude),
          longitude:
            typeof loc.longitude === "object" && "toNumber" in loc.longitude
  ...
  const addLocation = useCallback(
    async (location: Omit<VisitLocation, "id">) => {
      try {
        ...
        const dbLoc = {
          ...dbLocation,
          latitude:
            typeof dbLocation.latitude === "object" &&
            "toNumber" in dbLocation.latitude
              ? Number(dbLocation.latitude.toNumber())
              : Number(dbLocation.latitude),
          longitude:
            typeof dbLocation.longitude === "object" &&
            "toNumber" in dbLocation.longitude
        ...
        const errorMessage =
          err instanceof Error ? err.message : "訪問先の追加に失敗しました";
  ...
  const updateLocation = useCallback(
    async (id: string, updates: Partial<VisitLocation>) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "訪問先の更新に失敗しました";
        throw new Error(errorMessage);
      }
    },
    [updateMutation],
  ...
  const deleteLocation = useCallback(
    async (id: string) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "訪問先の削除に失敗しました";
        throw new Error(errorMessage);
      }
    },
    [deleteMutation],
